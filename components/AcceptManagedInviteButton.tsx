...
export default function AcceptManagedInviteButton({ token }: { token: string }) {
  ...
  const accept = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/managed-invites/${token}/accept`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to accept invite");
      router.push(`/dashboard/managed/${data.managedProjectId}`);
    } catch (err) {
