...
const BILLING_STATUS_LABEL: Record<
string,
{ text: string; color: string; bg: string }

> = {
 PENDING_SETUP: {
 text: "Never subscribed",
 color: "#F87171",
...
const label = monthDate.toLocaleDateString("en-NG", {
  month: "short",
  year: "2-digit",
});
...
const total = recentPayments
  .filter(
    (p) =>
      p.createdAt.getFullYear() === monthDate.getFullYear() &&
      p.createdAt.getMonth() === monthDate.getMonth()
  )
...
1,
...monthlyBreakdown.map((m) => m.total)
);
...
const totalCalendarPages = Math.max(
1,
Math.ceil(totalCalendars / PAGE_SIZE)
);
...
> <div className="mx-auto max-w-6xl px-6 py-12"> <div className="mb-8 flex items-center justify-between"> <div>
<p
...
Admin </p>
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/admin"
          className="text-sm text-white/40 underline hover:text-white"
        >
          ← Platform overview
        </Link>
      </div>
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {[
        ...
        },
        {
          ...
          value: totalCalendars.toLocaleString(),
        },
        {
          label: "Active subscriptions",
          value: activeCalendars.toLocaleString(),
        },
        {
          label: "Posts planned",
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-3">
      {[
        {
          label: "This month",
          value: monthAgg._sum.amountNgn ?? 0,
        },
        {
          label: "This year",
          value: yearAgg._sum.amountNgn ?? 0,
        },
        {
          label: "All time",
          value: allTimeAgg._sum.amountNgn ?? 0,
        },
      ].map((stat) => (
        <div
          key={stat.label}
    ...
    >
      <h2
        className="mb-5 text-sm font-semibold uppercase text-white/40"
        style={{ letterSpacing: "0.08em" }}
      >
        Content Workspace revenue, last 12 months
      </h2>
    ...
    <div
      className="mb-10 overflow-x-auto rounded-xl"
      style={{ background: COLOR.charcoal }}
    >
      <table className="w-full text-left text-sm">
        <thead>
          ...
          >
            <th className="px-4 py-3 font-semibold text-white/40">
              Manager
            </th>
            <th className="px-4 py-3 font-semibold text-white/40">
              Workspaces created
            ...
            calendars.map((cal) => {
              ...
              const billingMeta =
                BILLING_STATUS_LABEL[
                  cal.manager.contentWorkspaceBillingStatus
                ] ?? BILLING_STATUS_LABEL.PENDING_SETUP;

              const workspacePlan =
                cal.manager.contentWorkspacePlan === "STUDIO"
                  ? "Studio"
                  : cal.manager.contentWorkspacePlan === "CREATOR"
                    ? "Creator"
                    : "Not set";
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col items-start gap-1">
                      ...
                      {cal.manager.contentWorkspaceBillingCycle && (
                        <span className="text-[10px] text-white/30">
                          {cal.manager.contentWorkspaceBillingCycle ===
                          "ANNUAL"
                            ? "Annual"
                  ...
                  <td className="px-4 py-3 text-white/40">
                    {cal.createdAt.toLocaleDateString("en-NG", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })}
    ...
    {totalCalendarPages > 1 && (
      <div className="mt-6 flex items-center justify-between">
        <Link
          href={`/admin/social-calendars?page=${
            currentPage - 1
          }`}
        ...
        <span className="text-sm text-white/40">
          Page {currentPage} of {totalCalendarPages}
        </span>
