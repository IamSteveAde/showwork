...
export async function POST(req: NextRequest) {
  ...
  const { name, profileImageUrl, category, whatTheyDo, contact, portfolioUrl, whatsappNumber, wonFor, points, periodDate } = await req.json();
  ...
  const entry = await db.creativoLeaderboardEntry.create({
    data: {
      ...
      // Strip everything but digits, so whatever format the admin
      // types (with spaces, a "+", dashes) still ends up in the
      // plain-digits shape a wa.me link actually needs.
      whatsappNumber: whatsappNumber ? whatsappNumber.replace(/[^0-9]/g, "") || null : null,
      wonFor: wonFor.trim(),
